...
class ArticleController:
    ...
    async def create_article(self, article: ArticleCreateDTO, db: Session = Depends(get_db)):
        service = ArticleService(db)
        return service.create_article(article)

    async def read_article(self, article_id: int, db: Session = Depends(get_db)):
        service = ArticleService(db)
    ...
    async def update_article(self, article_id: int, article: ArticleUpdateDTO, db: Session = Depends(get_db)):
        service = ArticleService(db)
    ...
    async def delete_article(self, article_id: int, db: Session = Depends(get_db)):
        service = ArticleService(db)
    ...
    async def read_articles(self, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
        service = ArticleService(db)
